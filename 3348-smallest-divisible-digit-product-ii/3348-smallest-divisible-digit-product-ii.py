...
class Solution:
    ...
    def smallestNumber(self, num: str, t: int) -> str:
        ...
        primeCountPrefix = sum((self.FACTOR_COUNTS[int(c)] for c in num), start=collections.Counter())
                ...
                for biggerDigit in range(d + 1, 10):
                    ...
                    if sum(factorsAfterReplacement.values()) <= spaceAfterThisDigit:
                        ...
                        return (
                            ...
                            + str(biggerDigit)
                            + '1' * fillOnes
                            + ''.join(factor * freq for factor, freq in factorsAfterReplacement.items())
                        )
        factorCount = self._getFactorCount(primeCount)
        return (
    ...
    def _getPrimeCount(self, t: int) -> tuple[collections.Counter, bool]:
        ...
        for prime in [2, 3, 5, 7]:
            while t % prime == 0:
                ...
                count[prime] += 1
        return count, t == 1
    def _getFactorCount(self, count: collections.Counter) -> dict[str, int]:
        """ Packs prime factors greedily into the most compressed sequence of digits (2-9). """
        count8, remaining2 = divmod(count[2], 3)
        count9, count3 = divmod(count[3], 2)
        count4, count2 = divmod(remaining2, 2)
        count2, count3, count6 = ((0, 0, 1) if count2 == 1 and count3 == 1 else (count2, count3, 0))
        count2, count6, count3, count4 = ((1, 1, 0, 0) if count3 == 1 and count4 == 1 else (count2, count6, count3, count4))
